...
void analyze_kasiski(const char *text) {
        ...
        for (int j = i + 3; j < len - 3; j++) {
            if (strncmp(&text[i], &text[j], 3) == 0) {
				...
				if (match_count < MAX_MATCHES) {
					...
					printf("Намерено: %c%c%c на позиции %d и %d (Разстояние: %d)\n", 
	...
	printf("\n--- Резултати от гласуването за дължина на ключа ---\n");
	for (int len = 2; len <= 20; len++){
		if (votes[len] > 0) printf("Дължина %2d: %d гласа\n", len, votes[len]);
	}
	...
	printf("Топ 3 вероятни дължини:\n");
	for (int i = 2; i < 5; i++) {
		printf("%d-во място: Дължина %d (%d гласа)\n", i-1, results[i].length, results[i].count);
	}
}
